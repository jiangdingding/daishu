...
$.fn.extend({
	sSelect: function(opts) {
		...
		$('#'+selectId+' h4').empty().append($('#'+selectId+' option:selected').text()).addClass(className);
		
		var selectWidth=$('#'+selectId+' select').width()-15;
		
		if(opts.minWidth && selectWidth < opts.minWidth)
			selectWidth = opts.minWidth;
		
		$('#'+selectId+' h4').css({width:selectWidth}).addClass('item_on_0').data('index',0);
		...
		.bind("click",function(e){
            ...
            var disabled = $('#'+selectId+' select').attr('disabled');
            if(disabled){
                return;
            }
			if($('#'+selectId+' ul').css("display") == 'block'){
				...
				if(windowspace2<0 && windowspace-ulspace-$('#'+selectId+' h4').outerHeight(true)<0){
					$('#'+selectId+' ul').css({
						'height':windowspace-$('#'+selectId+' h4').outerHeight(true)-10,
						'overflow-y':"scroll"
					});
				}
				...
				$('#'+selectId+' li').click(function(e){
						...
						var h4 = $('#'+selectId+' h4');
						h4.html($('#'+selectId+' option:selected').text()).removeClass('item_on_'+h4.data('index')).addClass('item_on_'+selectIndex).data('index',selectIndex);
						$.fn.clearSelectMenu(selectId,selectZindex);
						e.stopPropagation();
						e.cancelbubble = true;
				})
	...
	setSelectValue:function(sID){
		...
		var currentItem = $('#'+sID+' option:selected'),
			h4 = $('#'+sID+' h4'),
			index = currentItem.data('index');

		h4.html(currentItem.text());
